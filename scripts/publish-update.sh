...
fail() {
  echo "update publish failed: $1" >&2
  exit 1
}

VERSION="${VERSION:-${1:-}}"
ARCHIVE="${ARCHIVE:-build/release/openflow.zip}"
...
REMOTE_TEMP="/tmp/openflow-update-${VERSION}-$$"
REMOTE_ARCHIVE="$REMOTE_TEMP/openflow.zip"
...
"$CLI" publish \
  ...
  --file "$ARCHIVE" \
  --file-name "openflow-$VERSION.zip" \
  --notes "$NOTES_FILE" \
  --rollout "$ROLLOUT" \
...
echo "Published openflow $VERSION to $CHANNEL at ${ROLLOUT}% rollout."
echo "Manifest: https://updates.jottly.ai/v1/apps/openflow/$CHANNEL/macos-aarch64/latest.json"
